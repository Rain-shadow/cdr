    ...
    def run(self):
        ...
        for i in task_choose_list:
            ...
            if i['over_status'] == 2:
                ...
                course_id = i.get("course_id") or re.match(r'.*/(.*)\.jpg', i["course_img_url"]).group(1)
                ...
                while True:
                    ...
                    Log.i(f"【{i['task_name']}】已完成。分数：{ClassTask.get_class_task_score(release_id)}")
                    if json_data["code"] == 20004:
                        break
                    if now_score <= ClassTask.get_class_task_score(release_id):
                        break
            else:
                Log.i(f"该【{i['task_name']}】任务未开始")
            # Log.v("")
        Log.i("本次全部任务已完成！")
        input("按回车键返回上一级")
